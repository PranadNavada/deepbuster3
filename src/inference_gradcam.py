...
def preprocess_image_for_model(img_path, target_size, model_name_hint=None, max_side=1024, verbose=False):
    img = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)
    if img is None:
        raise FileNotFoundError(f"Image not found: {img_path}")
    if verbose:
        print(f"[preprocess] original shape: {img.shape}")

    # ensure 3-channel BGR
    if len(img.shape) == 2:
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
        if verbose:
            print(f"[preprocess] converted grayscale to BGR: {img.shape}")
    elif img.shape[2] == 4:
        img = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)
        if verbose:
            print(f"[preprocess] stripped alpha channel: {img.shape}")
    ...
    if max_side and max(h, w) > max_side:
        ...
        if verbose:
            print(f"[preprocess] downscaled to: {img.shape}")

    img = cv2.resize(img, (int(target_size[1]), int(target_size[0])), interpolation=cv2.INTER_AREA)
    if verbose:
        print(f"[preprocess] resized to model input: {img.shape}")
...
def main():
    ...
    p.add_argument("--max-side", type=int, default=1024, help="Downscale images if larger than this size (0 disables)")
    p.add_argument("--verbose", action="store_true", help="Print preprocessing diagnostics")
    args = p.parse_args()
    ...
    img_array = preprocess_image_for_model(
        image_path,
        target_size,
